...
async function LoadScripts(scriptsArr) {
    if (scriptsArr.length === 1) {
        ...
        try {
            ...
            console.warn("[Construct] Unable to import script from blob: URL. Falling back to loading scripts sequentially, which could significantly increase loading time. Make sure blob: URLs are allowed for best performance.", err);
...
async function InitRuntime(data) {
    ...
    const workerDependencyScripts = data["workerDependencyScripts"].map(urlOrBlob => {
        ...
        if (urlOrBlob instanceof Blob)
            ...
            url = (new URL(url,runtimeBaseUrl)).toString();
        return url
    }
    );
        ...
        if (originalUrl === mainProjectScript)
            try {
                ...
                if (exportType === "preview" && !scriptsStatus[originalUrl])
                    ReportProjectMainScriptError(originalUrl, "main script did not run to completion", messagePort)
            } catch (err) {
